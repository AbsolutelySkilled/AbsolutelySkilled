<!-- Part of the Absolute-Human AbsolutelySkilled skill. Load this file when working with wave execution, scope management, failure handling, or anti-patterns. -->
...
2. **Non-blocking discovery** (nice-to-have, related improvement, cleanup): Do NOT absorb it. Add it to a `## Deferred Work` section on the board. Mention it in the CONVERGE summary. The user decides whether to start a new Absolute-Human session for it.
...
Every Absolute-Human task graph must include these three tasks as the final tasks, in this order:
...
| Not persisting board state | Always write to `.absolute-human/board.md` - it enables resume, audit, and handoff |
...
| Sequential execution when parallelism is possible | Always check the DAG for parallel opportunities - that's the whole point of Absolute-Human |
