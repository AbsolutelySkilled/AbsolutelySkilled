<!-- Part of the Absolute-Human AbsolutelySkilled skill. Load this file when the agent needs the full specification for the .absolute-human/board.md file, including format, status transitions, and examples. -->

# Board Format Specification

The `.absolute-human/board.md` file is the single source of truth for a Absolute-Human execution. It tracks everything from intake through convergence and is designed to be both human-readable and machine-parseable.
...
```
{project-root}/.absolute-human/board.md
```

The `.absolute-human/` directory may also contain:
...
When starting a new session and a `.absolute-human/board.md` exists:
...
### Resume Detection
At the start of any Absolute-Human invocation:
1. Check if `.absolute-human/board.md` exists
2. If yes, ask the user: "Found an existing Absolute-Human board. Resume it or start fresh?"
